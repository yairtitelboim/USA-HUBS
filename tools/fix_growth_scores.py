#!/usr/bin/env python3
"""
Fix Growth Potential Scores for All Counties

This script reads the existing fixed_county_scores.geojson file and adds or updates
the growth_potential_score field for all counties based on their obsolescence scores.
It's designed to be more robust and handle potential file corruption issues.

Usage:
    python fix_growth_scores.py [--input INPUT_FILE] [--output OUTPUT_FILE]
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logs/fix_growth_scores.log', mode='a')
    ]
)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Fix Growth Potential Scores for All Counties')
...
def fix_growth_scores(input_file, output_file, react_app_dir):
    """
    Fix growth potential scores for all counties.
    ...
    """
    logger.info(f"Fixing growth potential scores in {input_file}")
    ...
    try:
        ...
        counties_gdf.to_file(temp_file, driver='GeoJSON')
        logger.info(f"Saved {len(counties_gdf)} counties with updated growth potential scores to temporary file")

        # Verify the temporary file is valid
        try:
            test_gdf = gpd.read_file(temp_file)
            logger.info(f"Verified temporary file is valid with {len(test_gdf)} counties")

            # If valid, replace the original file
            shutil.move(temp_file, output_file)
            logger.info(f"Replaced original file with updated file")
            ...
            if react_app_dir:
                ...
                shutil.copy2(output_file, react_file)
        ...
        logger.error(f"Error fixing growth potential scores: {e}")
        return False

def main():
    """Main function to fix growth potential scores for all counties."""
    args = parse_args()

    logger.info("Starting to fix growth potential scores")

    # Try to fix growth potential scores, with retries if the file is being written to
    max_retries = 5
    retry_delay = 30  # seconds

    for retry in range(max_retries):
        try:
            if fix_growth_scores(args.input, args.output, args.react_app_dir):
                logger.info("Successfully fixed growth potential scores")
                break
            else:
                logger.error(f"Failed to fix growth potential scores (attempt {retry+1}/{max_retries})")
        ...
        logger.error(f"Failed to fix growth potential scores after {max_retries} attempts")
        sys.exit(1)
