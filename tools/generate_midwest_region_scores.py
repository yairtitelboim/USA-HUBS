#!/usr/bin/env python3
"""
Generate Midwest Region Scores

This script generates obsolescence scores for counties in the US Midwest region
and adds them to the county_scores.geojson file.

Usage:
    python generate_midwest_region_scores.py [--input INPUT_FILE] [--output OUTPUT_FILE]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Generate Midwest Region Scores')
...
def get_midwest_region_counties(shapefile_path, num_counties=20):
    """Get counties from the US Midwest region."""
    print(f"Loading county shapefile from {shapefile_path}...")
    
    # Load the county shapefile
    counties_gdf = gpd.read_file(shapefile_path)
    
    # Define Midwest region states by FIPS code
    # Midwest: IL, IN, MI, OH, WI, IA, KS, MN, MO, NE, ND, SD
    midwest_states = ['17', '18', '26', '39', '55', '19', '20', '27', '29', '31', '38', '46']
    
    # Filter counties in the Midwest region
    midwest_counties = counties_gdf[counties_gdf['STATEFP'].isin(midwest_states)]
    
    print(f"Found {len(midwest_counties)} counties in the Midwest region")
    
    # Randomly select counties if there are more than requested
    if len(midwest_counties) > num_counties:
        # Set seed for reproducibility
        random.seed(45)  # Different seed from other regions
        midwest_counties = midwest_counties.sample(num_counties, random_state=45)
    
    print(f"Selected {len(midwest_counties)} counties from the Midwest region")
    
    return midwest_counties

def generate_realistic_scores(counties_gdf):
    """Generate realistic obsolescence scores for counties."""
    # Midwest region has medium-low obsolescence scores (0.35-0.65)
    counties_gdf['obsolescence_score'] = np.random.uniform(0.35, 0.65, len(counties_gdf))
...
def update_county_scores(input_file, output_file, num_counties=20):
    """Update county scores with new Midwest region counties."""
    ...
    # Get counties from the Midwest region
    shapefile_path = 'data/tl_2024_us_county/tl_2024_us_county.shp'
    midwest_counties = get_midwest_region_counties(shapefile_path, num_counties)
    
    # Generate realistic scores for Midwest region counties
    midwest_counties = generate_realistic_scores(midwest_counties)
    ...
    new_counties = midwest_counties[~midwest_counties['GEOID'].isin(existing_geoids)]
    ...
    print(f"Added {len(new_counties)} new counties from the Midwest region")
