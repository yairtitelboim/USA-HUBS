#!/usr/bin/env python3
"""
Generate South Region Scores

This script generates obsolescence scores for counties in the US South region
and adds them to the county_scores.geojson file.

Usage:
    python generate_south_region_scores.py [--input INPUT_FILE] [--output OUTPUT_FILE]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Generate South Region Scores')
...
def get_south_region_counties(shapefile_path, num_counties=20):
    """Get counties from the US South region."""
    print(f"Loading county shapefile from {shapefile_path}...")

    # Load the county shapefile
    counties_gdf = gpd.read_file(shapefile_path)

    # Define South region states by FIPS code
    # South: DE, FL, GA, MD, NC, SC, VA, DC, WV, AL, KY, MS, TN, AR, LA, OK, TX
    south_states = ['10', '12', '13', '24', '37', '45', '51', '11', '54',
                    '01', '21', '28', '47', '05', '22', '40', '48']

    # Filter counties in the South region
    south_counties = counties_gdf[counties_gdf['STATEFP'].isin(south_states)]

    print(f"Found {len(south_counties)} counties in the South region")

    # Randomly select counties if there are more than requested
    if len(south_counties) > num_counties:
        # Set seed for reproducibility
        random.seed(42)
        south_counties = south_counties.sample(num_counties, random_state=42)

    print(f"Selected {len(south_counties)} counties from the South region")

    return south_counties

def generate_realistic_scores(counties_gdf):
    """Generate realistic obsolescence scores for counties."""
    # South region has higher obsolescence scores (0.65-0.95)
    counties_gdf['obsolescence_score'] = np.random.uniform(0.65, 0.95, len(counties_gdf))
...
def update_county_scores(input_file, output_file, num_counties=20):
    """Update county scores with new South region counties."""
    ...
    # Get counties from the South region
    shapefile_path = 'data/tl_2024_us_county/tl_2024_us_county.shp'
    south_counties = get_south_region_counties(shapefile_path, num_counties)

    # Generate realistic scores for South region counties
    south_counties = generate_realistic_scores(south_counties)
    ...
    new_counties = south_counties[~south_counties['GEOID'].isin(existing_geoids)]
    ...
    print(f"Added {len(new_counties)} new counties from the South region")
