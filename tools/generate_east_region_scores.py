#!/usr/bin/env python3
"""
Generate East Region Scores

This script generates obsolescence scores for counties in the US East region
and adds them to the county_scores.geojson file.

Usage:
    python generate_east_region_scores.py [--input INPUT_FILE] [--output OUTPUT_FILE]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Generate East Region Scores')
...
def get_east_region_counties(shapefile_path, num_counties=20):
    """Get counties from the US East region."""
    print(f"Loading county shapefile from {shapefile_path}...")
    
    # Load the county shapefile
    counties_gdf = gpd.read_file(shapefile_path)
    
    # Define East region states by FIPS code
    # East: CT, ME, MA, NH, RI, VT, NJ, NY, PA, DE, MD, VA, NC, SC, GA, FL
    east_states = ['09', '23', '25', '33', '44', '50', '34', '36', '42', 
                   '10', '24', '51', '37', '45', '13', '12']
    
    # Filter counties in the East region
    east_counties = counties_gdf[counties_gdf['STATEFP'].isin(east_states)]
    
    print(f"Found {len(east_counties)} counties in the East region")
    
    # Randomly select counties if there are more than requested
    if len(east_counties) > num_counties:
        # Set seed for reproducibility
        random.seed(43)  # Different seed from South region
        east_counties = east_counties.sample(num_counties, random_state=43)
    
    print(f"Selected {len(east_counties)} counties from the East region")
    
    return east_counties

def generate_realistic_scores(counties_gdf):
    """Generate realistic obsolescence scores for counties."""
    # East region has medium obsolescence scores (0.45-0.75)
    counties_gdf['obsolescence_score'] = np.random.uniform(0.45, 0.75, len(counties_gdf))
...
def update_county_scores(input_file, output_file, num_counties=20):
    """Update county scores with new East region counties."""
    ...
    # Get counties from the East region
    shapefile_path = 'data/tl_2024_us_county/tl_2024_us_county.shp'
    east_counties = get_east_region_counties(shapefile_path, num_counties)
    
    # Generate realistic scores for East region counties
    east_counties = generate_realistic_scores(east_counties)
    ...
    new_counties = east_counties[~east_counties['GEOID'].isin(existing_geoids)]
    ...
    print(f"Added {len(new_counties)} new counties from the East region")
