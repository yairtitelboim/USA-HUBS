#!/usr/bin/env python3
"""
Generate Northeast Region Scores

This script generates obsolescence scores for counties in the US Northeast region
and adds them to the county_scores.geojson file.

Usage:
    python generate_northeast_region_scores.py [--input INPUT_FILE] [--output OUTPUT_FILE]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Generate Northeast Region Scores')
...
def get_northeast_region_counties(shapefile_path, num_counties=20):
    """Get counties from the US Northeast region."""
    print(f"Loading county shapefile from {shapefile_path}...")
    
    # Load the county shapefile
    counties_gdf = gpd.read_file(shapefile_path)
    
    # Define Northeast region states by FIPS code
    # Northeast: CT, ME, MA, NH, RI, VT, NJ, NY, PA
    northeast_states = ['09', '23', '25', '33', '44', '50', '34', '36', '42']
    
    # Filter counties in the Northeast region
    northeast_counties = counties_gdf[counties_gdf['STATEFP'].isin(northeast_states)]
    ...
    if len(northeast_counties) > num_counties:
        # Set seed for reproducibility
        random.seed(47)  # Different seed from other regions
        northeast_counties = northeast_counties.sample(num_counties, random_state=47)
    
    print(f"Selected {len(northeast_counties)} counties from the Northeast region")
    
    return northeast_counties

def generate_realistic_scores(counties_gdf):
    """Generate realistic obsolescence scores for counties."""
    # Northeast region has low-medium obsolescence scores (0.30-0.60)
...
def update_county_scores(input_file, output_file, num_counties=20):
    """Update county scores with new Northeast region counties."""
    ...
    northeast_counties = get_northeast_region_counties(shapefile_path, num_counties)
    
    # Generate realistic scores for Northeast region counties
